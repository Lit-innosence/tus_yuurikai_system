...
use crate::adapters::repository::{AuthRepository, LockerAuthInfoRepository};
use crate::infrastructure::models::{Auth, LockerAuthInfo};
...
pub trait AuthUsecase: Sync + Send {
    async fn register(&self, service: &str, main_user: &UserInfo, co_user: &UserInfo, phase: &String, is_same: bool) -> Result<Auth, Status>;
...
impl AuthUsecaseImpl {
    pub fn new(auth_repository: Arc<dyn AuthRepository>, locker_auth_info_repository: Arc<dyn LockerAuthInfoRepository>) -> Self {
        AuthUsecaseImpl { auth_repository, locker_auth_info_repository }
    }
}
