...
// ページロード時にpairInfoがnullの場合は/nopageに遷移
//   useEffect(() => {
//     if (!pairInfo) {
//       navigate('/nopage');
//     }
...
const handleRegister = () => {
    if (selectedLocker) {
    // '/locker/register/confirm'にpairInfoとselectedLockerを渡す
    ...
    <Content style={{ padding: '50px 50px' }}>
        ...
        {loading ? (
            ...
            renderItem={(locker) => (
                ...
                >
                <List.Item.Meta
                    title={`ロッカーID: ${locker.locker_id}`}
                    description={`状態: ${locker.status === 'VACANT' ? '空き' : locker.status === 'OCCUPIED' ? '使用中' : '故障中'}`}
                />
                </List.Item>
            )}
            />
